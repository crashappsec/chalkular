...
docker-build-img-%:
	@echo "building image $(CHALKULAR_$(shell echo '$*' | tr '[:lower:]' '[:upper:]')_IMG)"
...
.PHONY: helm-build
helm-build: kubebuilder helmpatch-plugin yq helm-clean ## Generate a helm-chart using kubebuilder
	...
	@"$(YQ)" -ie '(.manager.serviceaccount | key) head_comment="Configure manager service account"' $(HELM_CHART_DIR)/values.yaml




.PHONY: helm-clean
helm-clean: ## Clean up the helm chart generated files
