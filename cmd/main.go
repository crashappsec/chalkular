...
import (
	...
	"github.com/crashappsec/chalkular/internal/artifacts/httpserver"
...
func main() {
	...
	flag.BoolVar(&secureArtifactsHTTP, "artifacts-http-secure", true,
		"If set, the artifacts HTTP endpoint is served securely via HTTPS. Use --artifacts-http-secure=false to use HTTP instead.")
	...
	flag.StringVar(&artifactsHttpCertName, "artifacts-http-cert-name", "tls.crt", "The name of the artifacts HTTP server certificate file.")
	flag.StringVar(&artifactsHttpCertKey, "artifacts-http-cert-key", "tls.key", "The name of the artifacts HTTP server key file.")
	...
	analyzer, err := analyze.NewScheduler(mgr.GetClient(), cfg)
	if err != nil {
		setupLog.Error(err, "unable to construct pipeline client")
		os.Exit(1)
	}

	if err = mgr.Add(analyzer); err != nil {
	...
	artifactsHttpServer, err := httpserver.NewServer(cfg, mgr.GetHTTPClient(), analyzer.GetClient(), artifactsHttpServerOptions)
	if err != nil {
